//Variables
	if tut_spawn_y == 0
	{
		tut_spawn_y = obj_bugu.y
	}
	if tut_spawn_x == 0
	{
		tut_spawn_x = obj_bugu.x + 150
	}
...
//Tutorial shadow sequence
	...
	{
		noshadowpls = true
		active_shadow_id = layer_sequence_create("Instances", tut_spawn_x, tut_spawn_y, shadow_seq)
	};
	...
	if (layer_sequence_exists("Instances", active_shadow_id))
	{
		if (layer_sequence_is_finished(active_shadow_id))
		{
			...
			active_falling_id = layer_sequence_create("Instances", tut_spawn_x, tut_spawn_y, falling_rock);
		};
	};
...
//Tutorial falling sequence
	if (layer_sequence_exists("Instances", active_falling_id))
	{
		...
		if _pos == 7
		{
			...
			cam_shake = 6;
			active_rock_id = instance_create_layer(tut_spawn_x, tut_spawn_y, "Instances", hzrd_obj);
		};
		...
		if (layer_sequence_is_finished(active_falling_id))
		{
			...
			instance_create_layer(tut_spawn_x, tut_spawn_y, "Instances", curr_obst);
